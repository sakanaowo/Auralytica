...
class StorageTests(unittest.TestCase):
    ...
    def test_initial_migration_creates_all_entities_and_reopens_without_data_loss(self):
        ...
        self.assertTrue({'imports', 'watch_events', 'videos', 'channel_decisions', 'download_batches', 'download_items', 'settings', 'player_playlists', 'player_playlist_tracks', 'player_favorites', 'player_track_cache', 'player_lyrics_cache'} <= tables)
    ...
    def test_v1_migration_preserves_review_downloads_and_adds_audit_tables(self):
        ...
        self.assertTrue({'audit_runs','audit_events','metadata_items','metadata_cache',
                         ...
                         'player_playlists','player_playlist_tracks','player_favorites','player_track_cache',
                         'player_lyrics_cache'} <= tables)
    ...
    def test_database_copy_preserves_manual_download_selection_and_rejects_old_code(self):
        ...
        with sqlite3.connect(copy_path) as untouched:
            self.assertEqual(untouched.execute('PRAGMA user_version').fetchone()[0], 6)

...
class DownloaderTests(unittest.TestCase):
    def setUp(self):
        ...
        (self.root / "watch-history.json").write_text(
            json.dumps(
                [
                    {
                        "titleUrl": f"https://youtu.be/{i:011d}",
                        "title": "Watched Song / dangerous: name",
    ...
    def test_failure_continues_and_retry_skips_successful_files(self):
        ...
        def flaky(video_id, *args):
            ...
            if video_id.endswith("1"):
                raise downloader.DownloadFailure("unavailable", "Video unavailable")
    ...
    def test_stop_keeps_partial_path_and_resume_uses_it(self):
        ...
        def continued(video_id, directory, *args):
            if video_id == "00000000000":
                self.assertEqual(directory, folders[0])
            return self.success(video_id, directory, *args)

        self.assertEqual(
            downloader.run_batch(self.db, self.batch, adapter=continued)["status"],
            "completed",
        )
    ...
    def test_missing_successful_file_downloads_again_in_new_batch(self):
        ...
        file = self.db.execute(
            "SELECT file_path FROM download_items WHERE video_id='00000000000'"
        ).fetchone()[0]
        Path(file).unlink()
        batch = batches.create_batch(self.db, self.root / "audio")
        self.assertEqual(batch["queued"], 1)
        self.assertEqual(
            downloader.run_batch(self.db, batch["batch_id"], adapter=self.success)[
                "status"
            ],
            "completed",
        )
...
"""
    ...
    def test_crash_after_publish_reuses_same_final_file(self):
        ...
        self.assertEqual(
            downloader.run_batch(self.db, self.batch, adapter=resume)["status"],
            "completed",
        )
        self.assertEqual(len(list((self.root / "audio").glob("*.webm"))), 3)
    ...
    def test_last_item_filesystem_failure_is_paused(self):
        ...
        self.assertEqual(
            downloader.run_batch(self.db, self.batch, adapter=full)["status"], "paused"
        )

    def test_orphan_downloader_keeps_worker_lock_until_child_exits(self):
        context = multiprocessing.get_context("spawn")
        parent = context.Process(
            target=child_with_worker_lock,
            args=(self.root / "state.sqlite3", self.batch, self.root),
        )
        parent.start()
        child_pid = None
        try:
            ...
            for _ in range(100):
                try:
                    ...
                    break
                except storage.BatchBusyError:
                    time.sleep(0.02)
            else:
                self.fail("Child lock not released after exit")
            self.assertEqual(result["status"], "paused")
        finally:
            if parent.is_alive():
                parent.terminate()
    ...
    def test_adapter_surfaces_protocol_errors_without_accepting_incomplete_output(self):
        ...
        error = dict(
            type="error", code="unavailable", message="fixture unavailable", fatal=False
        )
        ...
        with self.assertRaisesRegex(
            downloader.DownloadFailure, "không trả file hoàn tất"
        ):
    ...
    def test_ytdlp_error_summary_does_not_persist_urls_or_tokens(self):
        ...
        self.assertEqual(
            ...
            (
                "filesystem",
                "Không thể ghi file tải; kiểm tra quyền và dung lượng.",
                True,
            ),
        )

    def test_in_stream_clean_names_removes_video_id_and_tags(self):
        batches.pause_batch(self.db, self.batch)
        batch = batches.create_batch(
            self.db, self.root / "clean_audio", clean_names=True
        )["batch_id"]
        ...
        result = downloader.run_batch(self.db, batch, adapter=adapter)
        self.assertEqual(result["status"], "completed")
        published_files = list((self.root / "clean_audio").glob("*.webm"))
        self.assertEqual(len(published_files), 3)
        names = [f.name for f in published_files]
        # Verify no bracketed video IDs or official video tags in published names
        for name in names:
            self.assertNotIn("[0000000000", name)
            self.assertNotIn("Official", name)
            self.assertTrue(name.startswith("Alan Walker - Faded"))

    def test_in_stream_transcode_to_m4a_alac_with_cover_art(self):
        batches.pause_batch(self.db, self.batch)
        batch = batches.create_batch(
            self.db,
            self.root / "alac_audio",
        ...
        def adapter(video_id, directory, progress, stopped, lock_fd):
            ...
            thumb = directory / "audio.jpg"
            # Generate valid audio & image for ffmpeg
            subprocess.run(
                [
                    "ffmpeg",
                    "-y",
            ...
            return {
                ...
                "acodec": "pcm_s16le",
                "vcodec": "none",
                "title": "K-391 - Ignite (feat. Alan Walker) [Audio]",
                "artist": "K-391",
                "album": "Ignite Single",
                "release_year": "2018",
            }
        ...
        result = downloader.run_batch(self.db, batch, adapter=adapter)
        self.assertEqual(result["status"], "completed")
        published = list((self.root / "alac_audio").glob("*.m4a"))
        self.assertEqual(len(published), 3)
        for f in published:
            self.assertIn(".m4a", f.name)

...
`metadata.py` tạo snapshot từ import đang chọn, mặc định tối đa 50 ID xem nhiều nhất; hoặc nhận danh sách ID cụ thể đã có trong import. CLI `metadata collect` là thao tác gọi mạng rõ ràng. Thu thập tuần tự qua `ytmusicapi==1.12.2`, `get_song`, không đăng nhập. Provider key gồm version thư viện, parser `player-v1`, language `en`, location `VN`.
...
`storage.set_video_group` ghi `review_changed` trong cùng transaction với sửa nhóm, liên kết decision tự động gần nhất nếu có. Thao tác CLI/web dùng chung hàm này; có before/after effective group và user group, kể cả reset override. Chưa có ô nhập lý do sửa tay trong UI.
...
- CLI collect → cache → status → export dùng provider fixture để không gửi ID test ra mạng.
